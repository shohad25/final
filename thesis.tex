...
\usepackage{babel}
\begin{document}
\includepdf{ENGLISH_TITLE.pdf}
\includepdf{ENGLISH_TITLE_SIGN.pdf}
...
\begin{itemize}
%	\item[1]{Tzachi Hershkovich, et al. Probabilistic model for 3D interactive segmentation. Computer Vision and Image Understanding (CVIU), Special Issue on Probabilistic Models for Biomedical Image Analysis, 2016.}
	\item[1]{
	Ohad Shitrit, Tzachi Hershkovich ,Tamar Shalmon, Ilan Shelef, and Tammy Riklin Raviv. Probabilistic model for 3D interactive segmentation, In MICCAI workshop on Interactive Medical Image Computing (IMIC), 2014.}

	\item[2]{Tzachi Hershkovich ,Ohad Shitrit, Tammy Riklin Raviv et al., T.R., 2016. Probabilistic model for 3D interactive segmentation. Computer Vision and Image Understanding, 151, pp.47-60.
		Vancouver}
	
	\item[3]{Ohad Shitrit and Tammy Riklin Raviv. Accelerated Magnetic Resonance Imaging by Adversarial Neural Network. In Deep Learning in Medical Image Analysis and Multimodal Learning for Clinical Decision Support (pp. 30-38). Springer, Cham.}
	
	\item[4]{ TODO: add the Journal submission 
			}
...
This was a great journey, challenging and educational. Thanks to all the amazing people that support, encouraged and guided me through these years.

First of all, I would like to thank to my supervisor, Dr. Tammy Riklin-Raviv, for guiding me in the last four years. Starting from the my undergraduate final project, during my master and my thesis research. Thanks for letting me be a part of one of the most fascinating filed of research. I admire your professionalism, dedication and your curiosity, which always been an inspiration for me.
Tammy, thank you for the long hours of conversations, tips and guidances at any time,
explaining theory and ideas in a simple and easy way to understand. All these are made me a better researcher and engineer.

Second, I want to thank to my lab colleagues, for a fruitful brainstormings, technical support and most important, being my friends. Great thanks to Tzachi Hershkovich, that was my partner of my undergraduate final project, I have learned a lot from you.

Last but not least, I want to thank to my life partner, my beloved wife Lee, for all your support during this journey. You encouraged me to be who I am today.

% ------------- INTRO ------------- $
\chapter{Introduction}\label{chap:INTRO}

\section{Magnetic Resonance Imaging (MRI)}\label{intro:MRI_BASICS}
...
From MRI theory, we know that raw data collected from MR machines,
the k-space values, which are RF signals at a predefined range of
frequencies and phases. 

Let's define $G_{FE}$, $G{}_{PE}$ as the frequency and phase encoding
gradients respectively. K-space is in a spatial frequency domain.
The complex array of raw data points is treated as a two-dimensional
grid of points $\left(k_{x},k_{y}\right)$ where $k_{x},k_{y}$ are
defined as follows:
\begin{equation}
\begin{cases}
k_{x}=\gamma G_{FE}t\\
k_{y}=\gamma G_{PE}T_{PE}
\end{cases}
\end{equation}


$T_{PE}$ denotes the total time in which the respective gradient
is active and $r\gamma$ is the gyromagnetic constant. The desired
signal, a 2D MR image, is defined as the inverse Fourier transform
of the encoded measured signal $s_{0}$, where $\Omega\subset\mathbb{R^{\mathrm{2}}}$:
\begin{equation}
I\left(x,y\right)=\iintop_{\Omega}s_{0}\left(k_{x},k_{y}\right)e^{+j2\pi\left(k_{x}x+k_{y}y\right)}dk_{x}dk_{y}
\end{equation}


As known from signal processing theory, lack of information in k-space
will cause artifacts such as aliasing, blurring, and results in low
resolution MR images. In some cases, decreasing the acquisition time
is necessary for detecting temporal changes which can indicate certain
illnesses.

\section{MRI Acceleration}\label{intro:ACC_THE_ACQ}
\subsection{Classic methods}\label{intro:acc:CLASSIC_METHODS}
...
\subsection{Machine Learning Methods}\label{intro:acc:DATA_METHODS}
...
\section{K-space}\label{method:METHOD}
...
We use a standard discriminator architecture, composed of convolutional layers, batch normalization, and leaky-ReLU as suggested in \cite{radford2015unsupervised}. For the generator, we compose a dedicated architecture, detailed in Section~\ref{results:RANDOM_GAUS_RESULTS}. Both the discriminator and the generator inputs are two-channel image for the real and imaginary components. Figure~\ref{fig:architectures} illustrates their architectures. During the training phase, $k_{d}$ discriminator update steps are carried out for every step of the generator. The training ends when the discriminator cannot accurately distinguish between the reconstructed (``fake'') and the true inputs.
...
\section{Random Gaussian Sampling}\label{results:RANDOM_GAUS_RESULTS}
...
\section{Phase Encoding Sampling}\label{results:PHASE_ENC_RESULTS}

The training data consists of 55 $3D$ brain MRI (T1) scans of different patients, acquired by 1.5T MR machine with resolution of $256\times256$ pixels. We used $3k$ 2D slices for training and $1.2k$ for testing. In order to create k-space images for training, inverse orthonormal 2D FFT is applied to the fully-sampled MR images. We sample the k-space
images along the phase axis using Cartesian binary mask, while keeping 5\% from the centered phases, which is the area that contains 95\% of the energy. Data augmentation is created by random offsets of the proposed mask and image flipping. This leads us to reconstruction of the MR image from only 52\% of the original k-space data.

The generator is composed of $5$ blocks of CONV-BatchNorm-ReLU, with output channels $16,32,64,32,8,$ respectively. The last layer is CONV with two outputs channels (for real and imaginary parts). The discriminator is composed of $4$ blocks of CONV-Pool-BatchNorm-LReLU with output channels $8,16,32,16$ and one fully-connected layer. All CONV layers kernel size is $3\times3$. All weights was initialized by Xavier \cite{glorot2010understanding}. We used Adam solver with fixed learning rate of $0.0005$ and set $k_{g}$ to 20.

We compare the proposed method to reconstruction results obtained by using a conventional compressed sensing method CS-MRI \cite{lustig2007sparse} and Zero-filling. In addition, we trained a generator (G) using only L2 loss (CNN-L2). The same sampling mask were used for all cases. Visual comparison is shown in Figure~\ref{fig:results_phase}. MRIs reconstructed by using the suggested adversarial loss have stronger contrast and no significant aliasing or artifacts. Quantitative evaluation using PSNR values is presented in~Table~\ref{tbl:PSNR}. Note that the proposed method outperforms the other methods.

\begin{table}[H]
	\centering{}%
	\begin{tabular}{|c|c|c|}
		\hline 
		\textbf{PSNR}  & Mean  & std\tabularnewline
		\hline 
		\hline 
		Zero-filled  & 30.48  & 0.13\tabularnewline
		\hline 
		CS-MRI  & 36.78  & 0.4\tabularnewline
		\hline 
		CNN-L2  & 37.12  & 0.82\tabularnewline
		\hline 
		\textbf{Proposed}  & \textbf{37.95}  & 0.6\tabularnewline
		\hline 
	\end{tabular}\caption{\textcolor{black}{\footnotesize{}{}Error in PSNR}{\footnotesize{}\label{tbl:PSNR}}}
\end{table}
...
\section{Discussion}\label{summary::conclusions}
...
\includepdf{HEB_ABSTRACT.pdf}
\includepdf{HEB_TITLE_SIGN.pdf}
\includepdf{HEB_TITLE.pdf}
\end{document}